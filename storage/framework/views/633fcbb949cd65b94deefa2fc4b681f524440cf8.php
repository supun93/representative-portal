...
<button type='submit' class="btn btn-primary">Batch Type Transfer</button>
</form>
</div>
...
<div class="card-body row">
    ...
    <div class="form-group col-md-4">
        <label for="exam-group">From Batch Type</label>
        <select class='form-control' id='from_batch_type' name='from_batch_type' readonly>
        <option value='<?php echo e($data->batchStudent->batchtype->id); ?>'><?php echo e($data->batchStudent->batchtype->description); ?></option>
        </select>
    </div>
    <div class="form-group col-md-4">
        <label for="exam-group">To Batch Type</label>
        <select class='form-control' id='to_batch_type' name='to_batch_type' required='true'>
        <option value="">Select a batch type</option>
        <?php $__currentLoopData = $batchTypes->batchTypes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $batchTypes): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>{
            <?php if($batchTypes->batchType->id == $data->batchStudent->batchtype->id): ?> <?php continue; ?> <?php endif; ?>
        <option value='<?php echo e($batchTypes->batchType->id); ?>'><?php echo e($batchTypes->batchType->description); ?></option>
    ...
    $("#transfer_req").submit(function(event){
        ...
        confirm: {
            ...
            action: function(){
                $.ajax({
                type:'POST',
                url:"<?php echo e(route('request.batchtype-transfer.submit')); ?>",
                data : form_data,
                contentType: false,
                cache: false,
                processData:false,
...
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xamppp\htdocs\push student portal\kiu-student-portal\resources\views/transfer/batchtypetransfer.blade.php ENDPATH**/ ?>