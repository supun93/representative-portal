...
<input type='hidden' id='batch_id' name='batch_id' value='<?php echo e($data->batchStudent->batch_id); ?>'>
<input type='hidden' id='student_id' name='student_id' value='<?php echo e($data->range_id); ?>'>
<button type='submit' class="btn btn-primary">Main Group Transfer</button>
</form>
</div>
...
<div class="card-body row">
    ...
    <div class="form-group col-md-4">
        <label for="exam-group">From Main Group</label>
        <select class='form-control' id='from_group_id' name='from_group_id' readonly>
        <option value='<?php echo e($data->batchStudent->group->GroupID ?? ""); ?>'><?php echo e($data->batchStudent->group->GroupName ?? ""); ?></option>
        </select>
    </div>
    <div class="form-group col-md-4">
        <label for="exam-group">To Main Group</label>
        <select class='form-control' id='to_group_id' name='to_group_id' required='true'>
        <option value="">Select a group</option>
        <?php $__currentLoopData = $groupes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $groupes): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>{
            <?php if($groupes->id == $data->batchStudent->group->GroupID): ?> <?php continue; ?> <?php endif; ?>
        <option value='<?php echo e($groupes->id); ?>'><?php echo e($groupes->text); ?></option>
    ...
    $("#transfer_req").submit(function(event){
        ...
        confirm: {
            ...
            action: function(){
                $.ajax({
                type:'POST',
                url:"<?php echo e(route('request.group-transfer.submit')); ?>",
                data : form_data,
                contentType: false,
                cache: false,
                processData:false,
...
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xamppp\htdocs\push student portal\kiu-student-portal\resources\views/transfer/grouptransfer.blade.php ENDPATH**/ ?>